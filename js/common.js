  ...
  if (l > 1) {
    for (var n = 1; n < l; n++) {
      if (urls[n] != 'action=login' && urls[n] != 'action=logout') {
        q = urls[n].split('=');
        if (q.length == 2 && !(withoutParams[q[0]] && withoutParams[q[0]] == q[1])) {
          new_url[q[0]] = q[1];
        }
      }
    }
...
function initProvince(prefix, country) {
  prefix = prefix ? prefix + '_' : '';
  var o = {
    callBack: function() {
      $G(prefix + 'district').valid().value = this.district;
      $G(prefix + 'amphur').valid().value = this.amphur;
      $G(prefix + 'province').valid().value = this.province;
      $E(prefix + 'districtID').value = this.districtID;
      $E(prefix + 'amphurID').value = this.amphurID;
      $E(prefix + 'provinceID').value = this.provinceID;
    },
    onChanged: function() {
      $G(prefix + 'district').reset();
      $G(prefix + 'amphur').reset();
      $G(prefix + 'province').reset();
      $E(prefix + 'districtID').value = 0;
      $E(prefix + 'amphurID').value = 0;
      $E(prefix + 'provinceID').value = 0;
    }
  };
  initAutoComplete(
    prefix + 'district',
    WEB_URL + 'index.php/index/model/autocomplete/district?country=' + country,
  ...
  initAutoComplete(
    prefix + 'amphur',
    WEB_URL + 'index.php/index/model/autocomplete/amphur?country=' + country,
    'district,amphur,province',
    'location',
    o
  );
  initAutoComplete(
    prefix + 'province',
    WEB_URL + 'index.php/index/model/autocomplete/province?country=' + country,
    'district,amphur,province',
    'location',
    o
  );
